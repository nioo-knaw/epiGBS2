...
File | path
-----|-----
bedfile | ``r params$dir`/output_mapping/methylation.bed`
snpfile | ``r params$dir`/output_mapping/snp.vcf.gz`
...
mapping-log | ``r params$dir`/output_mapping/mapping_variantcalling.log`
...
cat $dir/output_mapping/mapping_variantcalling.log | awk '/watsonLog.final.out/,/% of reads unmapped: other/' | grep -E "(Uniquely mapped reads %|% of reads)" # gets mappings stats
```

Crick:
```{bash capture="mapping stats Crick-joined"}
cat $dir/output_mapping/mapping_variantcalling.log | awk '/crickLog.final.out/,/% of reads unmapped: other/' | grep -E "(Uniquely mapped reads %|% of reads)" # gets mappings stats
```
...
vcf <- read.vcfR(paste0(params$dir,'/output_mapping/snp.vcf.gz'), verbose = FALSE)
...
```{r}
methylation <- read.delim(paste0(params$dir,'/output_mapping/methylation.bed'))
